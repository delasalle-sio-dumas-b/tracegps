...
// Dernière mise à jour : 16/7/2018 par Dylan VALLÉE
...
class PointDeTrace extends Point
{
    ...
    public function PointDeTrace($unIdTrace, $unID, $uneLatitude, $uneLongitude, $uneAltitude,
                                 $uneDateHeure, $unRythmeCardio, $unTempsCumule, $uneDistanceCumulee, $uneVitesse)
    {
        ...
        $this->setIdTrace($unIdTrace);
        $this->setId($unID);
        $this->setDateHeure($uneDateHeure);
        $this->setRythmeCardio($unRythmeCardio);
        $this->setTempsCumule($unTempsCumule);
        $this->setDistanceCumulee($uneDistanceCumulee);
        $this->setVitesse($uneVitesse);
    }
    ...
    {
        $heures = self::getTempsCumule() / 3600;
        $minutes = (self::getTempsCumule() % 3600) / 60;
        $secondes = (self::getTempsCumule() % 3600) % 60;
